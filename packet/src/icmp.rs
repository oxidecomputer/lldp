    ...
    fn deparse(pkt: &Packet, hdr_size: usize) -> PacketResult<BytesMut> {
        let mut v = {
            if pkt.hdrs.ipv4_hdr.is_some() {
                ...
                return Err(crate::deparse_error("ICMP packet needs an IP header"));
            }
        };
    ...
    fn calculate_icmp_checksum(pkt: &Packet) -> u16 {
        ...
        if let Some(body) = &pkt.body {
            ...
            if let Ok(hdrs) = ipv4::Ipv4Hdr::parse(&mut pb) {
                ...
                let end = std::cmp::min(body.len(), pb.offset() + ipv4.ipv4_total_len as usize);
...
fn test_v4_checksum() {
    let data = [
        0x45u8, 0x00, 0x00, 0x34, 0x98, 0xf4, 0x00, 0x00, 0x01, 0x11, 0x4a, 0x20, 0xc0, 0xa8, 0x05,
        0xed, 0x08, 0x08, 0x08, 0x08, 0x98, 0xf3, 0x82, 0x9b, 0x00, 0x20, 0x0d, 0x7a, 0x00, 0x00,
...
fn test_v6_checksum() {
    let data = [
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x02, 0x08, 0x20, 0x78, 0x17,
        0x0f, 0x03, 0x04, 0x40, 0xc0, 0x00, 0x27, 0x8d, 0x00, 0x00, 0x09, 0x3a, 0x80, 0x00, 0x00,
        0x00, 0x00, 0x20, 0x01, 0x0d, 0xb8, 0xff, 0xff, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00,
    ];
    let icmp_hdr = IcmpHdr {
        icmp_type: 134,
